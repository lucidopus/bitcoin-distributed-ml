...
def main():
    ...
    sns.barplot(data=class_counts, x="Target", y="count", hue="Target", palette="viridis", legend=False, ax=ax1)
    ax1.set_title("Target Class Distribution (0=Down, 1=Up)")
    ax1.set_xlabel("Target Class")
    ax1.set_ylabel("Count")
    
    # Add count labels on top of bars
    for index, row in class_counts.iterrows():
        ax1.text(index, row['count'], str(row['count']), color='black', ha="center")
    ...
    print(f"Available columns for correlation: {available_cols}")
    
    if not available_cols:
        print("WARNING: No feature columns found for correlation matrix")
    else:
        # Select specific columns and sample
        pdf_corr = df.select(available_cols).sample(fraction=0.1, seed=42).toPandas()
        
        fig2, ax2 = plt.subplots(figsize=(10, 8))
        sns.heatmap(pdf_corr.corr(), annot=True, cmap="coolwarm", fmt=".2f", linewidths=0.5, ax=ax2)
    ...
    required_cols = ['Timestamp', 'Close', 'Feat_GK_Vol']
    missing_cols = [col for col in required_cols if col not in df.columns]
    
    if missing_cols:
        print(f"WARNING: Missing columns for volatility plot: {missing_cols}")
